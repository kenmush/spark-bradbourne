...
<form role="form">
    ...
    <!-- <input type="type" class="form-control" name="type" value="consumer" {{(Request::get('type')=='consumer')?'checked':null}} :class="{'is-invalid': registerForm.errors.has('type')}"> -->
    <!-- v-model="registerForm.type" -->
<input type="radio" value="consumer" name="type" v-model="registerForm.type" {{(Request::get('type')=='consumer')?'checked':null}}  />  consumer

<input type="radio" value="supplier" name="type" v-model="registerForm.type" {{(Request::get('type')=='supplier')?'checked':null}} /> supplier
            <span class="invalid-feedback" v-show="registerForm.errors.has('type')">
    ...
    <!-- category -->
    <div style="display:none" class="form-group row" >
        <label class="col-md-4 col-form-label text-md-right">{{__('category Type')}}</label>

        <div class="col-md-6">
<input type="radio" value="1" name="category" v-model="registerForm.category"  {{(Request::get('category')=='1')?'checked':null}}  />  1

<input type="radio" value="2" name="category" v-model="registerForm.category" {{(Request::get('category')=='2')?'checked':null}} /> 2
            <span class="invalid-feedback" v-show="registerForm.errors.has('category')">
                @{{ registerForm.errors.get('category') }}
            </span>
        </div>
    </div> 
            ...
            <div class="col-md-6 offset-md-4">
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="terms">
                        {!! __('I Accept :linkOpen The Terms Of Service123 :linkClose', ['linkOpen' => '<a href="/terms" target="_blank">', 'linkClose' => '</a>']) !!}
                    </label>
